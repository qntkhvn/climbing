...
Table 1 and Figure 1 are numerical and visual summaries of climbing total score obtained from our simulated data for the qualification round.
...
qual %>% 
  group_by(rank) %>% 
  summarize(avg_adv_score = mean(score)) %>% 
  filter(rank <= 8) %>% 
...
```{r, fig.cap = "Boxplots of scoring distribution for every qualification rank", eval = FALSE}
qual %>% 
  mutate(rank = factor(rank)) %>% 
  ggplot(aes(x = score, y = rank)) +
  geom_boxplot()
```
