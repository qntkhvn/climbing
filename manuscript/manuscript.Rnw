...
\subsections{Simulations}
\subsubsection{uniform ranks}
 - Distributions of points for qualifying and finals given uniform rankings. 
 - Conditional probabilities.  
 \subsubsections{Leave one out analysis} 
 - For n finalists, how often is the ranking preserved when a single climber is removed.  - Independence of Irrelevant groups.  (IIG)
